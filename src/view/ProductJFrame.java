...
import serviec.CategoryServiec;
...
public class ProductJFrame extends javax.swing.JFrame {
    ...
    private ProductServiec prSvc;
    private CategoryServiec cSvc;

    public ProductJFrame() {
        initComponents();
        this.prSvc = new ProductServiec();
        this.cSvc = new CategoryServiec();
        this.loadtable();
        this.loadCombobox();
        this.clearForm();
    }
    ...
    private void btnTimActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnTimActionPerformed
        ...
        String ma = this.txtMaSP.getText().trim();
        if (ma.length() == 0) {
            JOptionPane.showConfirmDialog(this, "Mã sản phẩm không được để trống");
            return;
        }
        int dem = -1;
        ArrayList<Product> lst = this.prSvc.getAll();
        for (Product x : lst) {
            dem++;
            Product p = (Product) x;
            if (p.getProduct_id().equals(ma)) {
                this.txtGia.setText(p.getCurrent_price() + "");
                this.txtMaSP.setText(p.getProduct_id());
                this.txtSoLuong.setText(p.getQuantity()+"");
                this.txtTenSP.setText(p.getProduct_name());

                int slDanhMuc = this.cbbDanhMuc.getItemCount();
                for (int i = 0; i < slDanhMuc; i++) {
                    Category c = this.cbbDanhMuc.getItemAt(i);
                    if (c.getId() == p.getCategory_id().getId()) {
                        this.cbbDanhMuc.setSelectedIndex(i);
                    }
                }
                JOptionPane.showMessageDialog(this, "Đã tìm thấy sản phẩm có mã "+ p.getProduct_id());
                this.tblSanPham.setRowSelectionInterval(dem, dem);
                return;
            }
        }
        JOptionPane.showMessageDialog(this, "Không tìm thấy sảm phẩm có mã trùng hợp");
        this.clearForm();
